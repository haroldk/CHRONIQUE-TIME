...
class AdminController extends Zend_Controller_Action {
	...
	function chroniquesAction() {
		...
		$select->setIntegrityCheck(false)
				->from(array('p'=>'chroniques_pages'), array('DISTINCT(p.id) as page','p.id_chronique as chronique','p.page as numero'))
		...
		$rechpage = new Application_Form_AdminRecherche('page');
		$this->view->formp = $rechpage;
	...
	function newchroniqueAction() {
		...
		if ($this->_request->isPost()) {
			...
			}else echo "<div class='alert alert-error' style='width:960px; margin: 30px auto;'>formData non-valide !</div>";
		}
	...
	function newpageAction() {
		...
		if ($this->_request->isPost()) {
			...
			if( $form->isValid($formData)) {
				...
				$select = $page->select();
				$select->setIntegrityCheck(false)
						->from( array('p'=>'chroniques_pages'), array('count(p.id) as page_num') )
						->where('id_chronique = ?',$form->getValue('id_chronique'));
				$page = $page->fetchRow($select);

				$row->id_chronique = $form->getValue('id_chronique');
				$row->page = $page->page_num + 1;
	...
	function modifAction() {
		...
		//traitement form->bdd chroniqueur/lecteur	
			...
			($_POST['mdp'] == "" ? $_POST['mdp'] = $ch->mdp : $_POST['mdp'] = md5($_POST['mdp'])) ;
			$formData = $this->_request->getPost();
			if( $form->isValid($formData)) {
			...
			if( $form->isValid($formData)) {
				...
				if($form->getValue('id') == 'chronique'){
					...
					$select->setIntegrityCheck(false)
							...
							->orWhere( 'c.titre LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'c.titre LIKE ?', '%'.$form->getValue('search').'%' )
							->joinLeft(array('t'=>'themes'), 'c.id_theme = t.id', array('t.libele as theme'))
							->joinLeft(array('k'=>'users'), 'c.id_chroniqueur = k.id', array('k.pseudo as chroniqueur'))
							->joinLeft(array('q'=>'commentaires'), 'c.id = q.id_chronique', array('COUNT(DISTINCT q.id) as commentaires'))
				...
				}elseif ($form->getValue('id') == 'page'){

					$pages = new Application_Model_DbTable_Chroniquespages();
					$select = $pages->select();
					$select->setIntegrityCheck(false)
							->from(array('p'=>'chroniques_pages'), array('DISTINCT(p.id) as page','p.id_chronique as chronique','p.page as numero'))
							->where( 'p.texte LIKE ?', '%'.$form->getValue('search') )
							->orWhere( 'p.texte LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'p.texte LIKE ?', '%'.$form->getValue('search').'%' )
							->joinLeft(array('c'=>'chroniques'), 'c.id = p.id_chronique',array('c.titre as titre', 'c.date_publication as date'))
							->joinLeft(array('t'=>'themes'), 'c.id_theme = t.id', array('t.libele as theme'))
							->joinLeft(array('k'=>'users'), 'c.id_chroniqueur = k.id', array('k.pseudo as chroniqueur'))
							...
							->limitPage($current, '12')
							->order('p.id_chronique DESC');
					if($pages = $pages->fetchAll($select)){

						$this->view->search = $pages;
						$this->view->type = $form->getValue('id');
					...
					$select->setIntegrityCheck(false)
							->from('themes')
							->where( 'libele LIKE ?', '%'.$form->getValue('search') )
							->orWhere( 'libele LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'libele LIKE ?', '%'.$form->getValue('search').'%' )
							->limitPage($current, '12');
					if($themes = $themes->fetchAll($select)){
						
						$this->view->search = $themes;
						$this->view->type = $form->getValue('id');
					...
					$select->setIntegrityCheck(false)
							->from('users')
							->where('chroniqueur = 1')
							->where( 'pseudo LIKE ?', '%'.$form->getValue('search') )
							->orWhere( 'pseudo LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'pseudo LIKE ?', '%'.$form->getValue('search').'%' )
							->where( 'nom LIKE ?', '%'.$form->getValue('search') )
							->orWhere( 'nom LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'nom LIKE ?', '%'.$form->getValue('search').'%' )
							->where( 'prenom LIKE ?', '%'.$form->getValue('search') )
							->orWhere( 'prenom LIKE ?', $form->getValue('search').'%' )
							->orWhere( 'prenom LIKE ?', '%'.$form->getValue('search').'%' )
							->limitPage($current, '12')
							->order('id DESC');
					$chroniqueurs = $chroniqueurs->fetchAll($select);
					$this->view->search = $chroniqueurs;
					$this->view->type = $form->getValue('id');
